...
content_id: bd01d60d4cc3a06563662b1f228e26e8e54412dc5479d95207d4e345a170b4c6
---

# file: src/concepts/TaggingConcept.test.ts
...
import TaggingConcept from "@concepts/TaggingConcept.ts"; // Corrected import path and class name
...
Deno.test("TaggingConcept: Add Tag - requires (duplicate)", async () => {
  ...
  assertEquals(
    ...
    "Tag by user for this book and label already exists.",
...
Deno.test("TaggingConcept: Remove Tag - success", async () => {
  ...
  assertNotEquals((result as { error: string }).error, "Tag not found.", "Should not return an error for existing tag");
...
Deno.test("TaggingConcept: Remove Tag - non-existent", async () => {
  ...
  assertEquals((result as { error: string }).error, "Tag not found.", "Error message should indicate tag not found");
...
Deno.test("TaggingConcept: Mark Private - success", async () => {
  ...
  assertNotEquals((result as { error: string }).error, "Tag not found.", "Should not return an error for existing tag");
  assertNotEquals((result as { error: string }).error, "Tag is already private.", "Should not return error for already private tag");
...
Deno.test("TaggingConcept: Mark Private - requires (already private)", async () => {
  ...
  // Requires: Expect an error
  assertExists((result as { error: string }).error, "Should return an error for already private tag");
  assertEquals(
    (result as { error: string }).error,
    "Tag is already private.",
    "Error message should indicate tag is already private",
  );

  console.log("Marking already private tag correctly failed.");
  await client.close();
});
...
Deno.test("TaggingConcept: Mark Public - success", async () => {
  ...
  assertNotEquals((result as { error: string }).error, "Tag not found.", "Should not return an error for existing tag");
  assertNotEquals((result as { error: string }).error, "Tag is already public.", "Should not return error for already public tag");
...
Deno.test("TaggingConcept: Mark Public - requires (already public)", async () => {
  ...
  // Requires: Expect an error
  assertExists((result as { error: string }).error, "Should return an error for already public tag");
  assertEquals(
    (result as { error: string }).error,
    "Tag is already public.",
    "Error message should indicate tag is already public",
  );

  console.log("Marking already public tag correctly failed.");
  await client.close();
});
...
Deno.test("TaggingConcept: Mark Private/Public - non-existent tag", async () => {
  ...
  assertEquals((result as { error: string }).error, "Tag not found.");
...
Deno.test("TaggingConcept: Query _getBooksbyLabel", async () => {
  ...
  const booksWithSciFi_UserA = await tagging._getBooksbyLabel({ user: userA, labels: ["sci-fi"] });
  ...
  assertEquals(bookIdsSciFi_UserA, [bookX, bookY], "Books found for 'sci-fi' by UserA are correct");

  // Query 2: UserB searches for "sci-fi"
  console.log(`User ${userB} searching for label 'sci-fi'`);
  const booksWithSciFi_UserB = await tagging._getBooksbyLabel({ user: userB, labels: ["sci-fi"] });
  ...
  assertEquals(bookIdsSciFi_UserB, [bookX, bookY], "Books found for 'sci-fi' by UserB are correct");

  // Query 3: UserA searches for "personal_sci-fi"
  console.log(`User ${userA} searching for label 'personal_sci-fi'`);
  const booksWithPersonalSciFi_UserA = await tagging._getBooksbyLabel({ user: userA, labels: ["personal_sci-fi"] });
  ...
  const booksWithPersonalSciFi_UserB = await tagging._getBooksbyLabel({ user: userB, labels: ["personal_sci-fi"] });
  assertEquals(
  ...
  // Query 5: UserA searches for multiple labels ("sci-fi", "fantasy", "personal_sci-fi")
  ...
  assertEquals(booksWithMultipleLabels_UserA.length, 4, "UserA should find 2 public sci-fi, 1 public fantasy, and their 1 private personal_sci-fi tag");
  const bookLabelsMultiple_UserA = booksWithMultipleLabels_UserA.map((t) => `${t.book}-${t.label}`).sort();
  assertEquals(bookLabelsMultiple_UserA, [
    `${bookX}-fantasy`,
    `${bookX}-sci-fi`,
    `${bookY}-sci-fi`,
    `${bookZ}-personal_sci-fi`,
  ], "Combined labels for UserA are correct");
...
Deno.test("TaggingConcept: Principle Trace", async () => {
  ...
  let classicTags = await tagging._getBooksbyLabel({ user: user1, labels: ["classic"] }); // user doesn't matter for public tags
  assertEquals(classicTags.length, 1);
  assertEquals(classicTags[0].book, book1);
  console.log(`Found ${classicTags.length} book(s) with 'classic' tag: ${classicTags.map(t => t.book).join(", ")}`);

  // Display books tagged 'adventure'
  console.log(`\nDisplaying books with 'adventure' tag (public search):`);
  let adventureTags = await tagging._getBooksbyLabel({ user: user1, labels: ["adventure"] });
  assertEquals(adventureTags.length, 1);
  assertEquals(adventureTags[0].book, book1);
  console.log(`Found ${adventureTags.length} book(s) with 'adventure' tag: ${adventureTags.map(t => t.book).join(", ")}`);
  ...
  let user1_scifi_books = await tagging._getBooksbyLabel({ user: user1, labels: ["sci-fi"] });
  assertEquals(user1_scifi_books.length, 1);
  assertEquals(user1_scifi_books[0].book, book2);
  console.log(`User ${user1} found ${user1_scifi_books.length} book(s) with 'sci-fi': ${user1_scifi_books.map(t => t.book).join(", ")}`);

  // User 2 searches for 'dystopian'
  console.log(`\nUser ${user2} searches for 'dystopian':`);
  let user2_dystopian_books = await tagging._getBooksbyLabel({ user: user2, labels: ["dystopian"] });
  assertEquals(user2_dystopian_books.length, 1);
  assertEquals(user2_dystopian_books[0].book, book3);
  console.log(`User ${user2} found ${user2_dystopian_books.length} book(s) with 'dystopian': ${user2_dystopian_books.map(t => t.book).join(", ")}`);

  // User 1 searches for their private tag 'private_note' on book3
  console.log(`\nUser ${user1} searches for their private tag 'private_note':`);
  let user1_private_note_books = await tagging._getBooksbyLabel({ user: user1, labels: ["private_note"] });
  assertEquals(user1_private_note_books.length, 1);
  assertEquals(user1_private_note_books[0].book, book3);
  console.log(`User ${user1} found ${user1_private_note_books.length} book(s) with 'private_note': ${user1_private_note_books.map(t => t.book).join(", ")}`);
  ...
  let user2_private_note_books = await tagging._getBooksbyLabel({ user: user2, labels: ["private_note"] });
