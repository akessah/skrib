...
[@tagging-concept](../skrib/TagConcept.md)

[@tagging-implementation](../../../src/concepts/skrib/Tagging.ts)

[@tagging-tests](../../../src/concepts/skrib/Tagging.test.ts)

Write additional tests only for _getLabelsByUser and _getLabelsByBook
...
const userC = "user:Charlie" as ID; // Added for more comprehensive testing
const bookX = "book:1984" as ID;
const bookY = "book:Dune" as ID;
const bookZ = "book:Foundation" as ID;
const bookW = "book:Solaris" as ID; // Added for more comprehensive testing
...
Deno.test.afterEach(async () => {
  if (client) {
        await client.close();
        await new Promise((resolve) => setTimeout(resolve, 10)); // Give a moment for connection to close
    }
...
Deno.test("TaggingConcept: Query _getTagsByUser", async () => {
  ...
  const tagsForNonExistentUser = await tagging._getTagsByUser({ user: userC });
...
Deno.test("TaggingConcept: Query _getLabelsByUser", async () => {
  console.log("--- Test: Query _getLabelsByUser ---");

  // Setup: UserA adds various tags, some with duplicate labels
  await tagging.addTag({ user: userA, label: "sci-fi", book: bookX });
  await tagging.addTag({ user: userA, label: "fantasy", book: bookY });
  await tagging.addTag({ user: userA, label: "sci-fi", book: bookZ }); // Another sci-fi tag by userA
  const privateTagId = (await tagging.addTag({ user: userA, label: "personal_note", book: bookW })) as { tag: ID };
  await tagging.markPrivate({ tag: privateTagId.tag }); // Make private

  // UserB adds a tag
  await tagging.addTag({ user: userB, label: "adventure", book: bookX });

  console.log(`User ${userA} added: 'sci-fi' (bookX), 'fantasy' (bookY), 'sci-fi' (bookZ), 'personal_note' (bookW, private).`);
  console.log(`User ${userB} added: 'adventure' (bookX).`);

  // Query 1: Get labels and counts for UserA
  console.log(`\nQuerying labels for User ${userA}`);
  const labelsForUserA = await tagging._getLabelsByUser({ user: userA });
  assertEquals(labelsForUserA.length, 3, "UserA should have 3 unique labels");

  // Expected labels and counts, sorted by count descending
  ...
  // Sort actual results for stable comparison
  const sortedLabelsA = labelsForUserA.sort((a, b) => {
    if (b.count !== a.count) return b.count - a.count;
    return a.label.localeCompare(b.label); // Secondary sort by label for consistency
  });

  assertEquals(sortedLabelsA, expectedLabelsA, "Labels and counts for UserA should match expected");
  console.log(`Labels for User ${userA}:`, sortedLabelsA);

  // Query 2: Get labels and counts for UserB
  console.log(`\nQuerying labels for User ${userB}`);
  const labelsForUserB = await tagging._getLabelsByUser({ user: userB });
  assertEquals(labelsForUserB.length, 1, "UserB should have 1 unique label");
  assertEquals(labelsForUserB[0], { label: "adventure", count: 1 }, "Labels and counts for UserB should match expected");
  console.log(`Labels for User ${userB}:`, labelsForUserB);

  // Query 3: Get labels for a user with no tags
  console.log(`\nQuerying labels for User ${userC} (no tags)`);
  const labelsForUserC = await tagging._getLabelsByUser({ user: userC });
  assertEquals(labelsForUserC.length, 0, "Non-existent user or user with no tags should return an empty array");
  console.log(`Labels for User ${userC}:`, labelsForUserC);

  console.log("Query _getLabelsByUser tests passed.");
});

Deno.test("TaggingConcept: Query _getLabelsByBook", async () => {
  console.log("--- Test: Query _getLabelsByBook ---");

  // Setup: Tags on bookX by different users, including public/private
  await tagging.addTag({ user: userA, label: "classic", book: bookX });
  await tagging.addTag({ user: userA, label: "fiction", book: bookX });
  await tagging.addTag({ user: userB, label: "classic", book: bookX }); // Another user with 'classic' tag
  const userAPrivateTagId = (await tagging.addTag({ user: userA, label: "private_review", book: bookX })) as { tag: ID };
  await tagging.markPrivate({ tag: userAPrivateTagId.tag });

  // Add some tags to bookY to ensure isolation
  await tagging.addTag({ user: userB, label: "mystery", book: bookY });

  console.log(`Book ${bookX} tags: UserA: 'classic', 'fiction', 'private_review' (private); UserB: 'classic'.`);
  console.log(`Book ${bookY} tags: UserB: 'mystery'.`);

  // Query 1: Get labels and counts for UserA on bookX
  console.log(`\nQuerying labels for Book ${bookX} as User ${userA}`);
  const labelsForBookX_UserA = await tagging._getLabelsByBook({ user: userA, book: bookX });
  assertEquals(labelsForBookX_UserA.length, 3, "UserA should see 3 unique labels for BookX (public + their own private)");

  const expectedLabelsBookX_UserA = [
    { label: "classic", count: 2 },
    { label: "fiction", count: 1 },
    { label: "private_review", count: 1 },
  ];

  const sortedLabelsBookX_UserA = labelsForBookX_UserA.sort((a, b) => {
    if (b.count !== a.count) return b.count - a.count;
    return a.label.localeCompare(b.label);
  });
  assertEquals(sortedLabelsBookX_UserA, expectedLabelsBookX_UserA, "Labels and counts for UserA on BookX should match expected");
  console.log(`Labels for Book ${bookX} (as User ${userA}):`, sortedLabelsBookX_UserA);

  // Query 2: Get labels and counts for UserB on bookX (should not see UserA's private tag)
  console.log(`\nQuerying labels for Book ${bookX} as User ${userB}`);
  const labelsForBookX_UserB = await tagging._getLabelsByBook({ user: userB, book: bookX });
  assertEquals(labelsForBookX_UserB.length, 2, "UserB should see 2 unique labels for BookX (only public)");

  const expectedLabelsBookX_UserB = [
    { label: "classic", count: 2 },
    { label: "fiction", count: 1 },
  ];

  const sortedLabelsBookX_UserB = labelsForBookX_UserB.sort((a, b) => {
    if (b.count !== a.count) return b.count - a.count;
    return a.label.localeCompare(b.label);
  });
  assertEquals(sortedLabelsBookX_UserB, expectedLabelsBookX_UserB, "Labels and counts for UserB on BookX should match expected");
  console.log(`Labels for Book ${bookX} (as User ${userB}):`, sortedLabelsBookX_UserB);

  // Query 3: Get labels for a book with no tags
  console.log(`\nQuerying labels for Book ${bookW} (no tags) as User ${userA}`);
  const labelsForBookW_UserA = await tagging._getLabelsByBook({ user: userA, book: bookW });
  assertEquals(labelsForBookW_UserA.length, 0, "Book with no visible tags should return an empty array");
  console.log(`Labels for Book ${bookW} (as User ${userA}):`, labelsForBookW_UserA);

  console.log("Query _getLabelsByBook tests passed.");
});
...
Deno.test("TaggingConcept: Principle Trace", async () => {
  ...
  let classicBooks = await tagging._getBooksByLabel({ user: user1, labels: ["classic"], type: 'union' }); // user doesn't matter for public tags
  assertEquals(classicBooks.length, 1);
  assertEquals(classicBooks[0].book, book1);
  console.log(`Found ${classicBooks.length} book(s) with 'classic' tag: ${classicBooks.map(b => b.book).join(", ")}`);

  // Display books tagged 'adventure'
  console.log(`\nDisplaying books with 'adventure' tag (public search):`);
  let adventureBooks = await tagging._getBooksByLabel({ user: user1, labels: ["adventure"], type: 'union' });
  assertEquals(adventureBooks.length, 1);
  assertEquals(adventureBooks[0].book, book1);
  console.log(`Found ${adventureBooks.length} book(s) with 'adventure' tag: ${adventureBooks.map(b => b.book).join(", ")}`);
  ...
  console.log(`User ${user1} found ${user1_scifi_books.length} book(s) with 'sci-fi': ${user1_scifi_books.map(b => b.book).join(", ")}`);
  ...
  console.log(`User ${user2} found ${user2_dystopian_books.length} book(s) with 'dystopian': ${user2_dystopian_books.map(b => b.book).join(", ")}`);
  ...
  console.log(`User ${user1} found ${user1_private_note_books.length} book(s) with 'private_note': ${user1_private_note_books.map(b => b.book).join(", ")}`);
