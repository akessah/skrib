---
timestamp: 'Sat Oct 18 2025 17:45:43 GMT-0400 (Eastern Daylight Time)'
content_id: 8fbdab6615046fedc42580270a67a6866438395111c5034ebc0a64c9c468cb28
---
...
export default class PostingConcept {
    ...
    async _getTagsByBook({ user, book }: {user: User; book: Book}): Promise<Tags[]> {
        const tags = this._getAllPublicTags({});
    ...
    async _getTagsByUser({ user }: {user: User}): Promise<Tags[]> {
        return await this.tags.find({user}).toArray();
        // throw new Error("not implemented");
    }
