...
Deno.test("Principle: A user receives and can view a notification", async () => {
...
Deno.test("Action: notify (user: User, message: String)", async (t) => {
...
Deno.test("Action: read (notification: Notification)", async (t) => {
  console.log("\n--- Testing read action ---");
...
Deno.test("Queries: _getNotificationsByUser, _getReadNotificationsByUser, _getUnreadNotificationsByUser, _getAllNotifications", async (t) => {
  const [db, client] = await testDb();
  const notifyingConcept = new NotifyingConcept(db);
